...
(async function(){
    try{
        ...
        for(let i=0 ; i<challenges.length ; i++){
            ...
            await addChallenge(addChallengeLink , newTab, challenges[i]);
        }
...
async function addChallenge(link , newTab, challenge) {
    ...
    console.log(Object.keys(challenge)[0]);
    // let alltabs = await newTab.$$(".block.span12.profile");
    // for (let i = 0; i < alltabs.length; i++) {
    //     await newTab.type(alltabs[i], challenge[i]);
    // }
    //let alltab = await newTab.$$(".block.span12.profile-input input");
    //await newTab.waitForNavigation({waitUntil : "networkidle2"});
    //await newTab.type(alltab[0], challenge[0]);

    //Object.keys(chanllenge);
}